...
class lib_noisefree():
    def __init__(self, lib_dir, lib_datalm, lib_lencmb, cl_transf, cache_sims=False):
        ...
        if not os.path.exists(lib_dir + '/sim_hash_noisefree.pk') and pbs.rank == 0:
            pk.dump(self.hashdict(), open(lib_dir + '/sim_hash_noisefree.pk', 'w'))
        pbs.barrier()
        hash_check(self.hashdict(), pk.load(open(lib_dir + '/sim_hash_noisefree.pk', 'r')))
